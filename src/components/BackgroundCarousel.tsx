...
export default function BackgroundCarousel({
  ...
  useEffect(() => {
    ...
    const run = () => { timer.current = window.setInterval(() => {
      setFade(true); window.setTimeout(() => { setI((p) => (p + 1) % images.length); setFade(false); }, 250);
    }, intervalMs); };
    run();
  ...
  return (
    <div ref={ref} className={`absolute inset-0 ${className}`} aria-hidden="true">
      <picture className="block h-full w-full">
        ...
        <img key={i} src={cur.src} alt={cur.alt ?? ""} loading={i===0?"eager":"lazy"} fetchPriority={i===0?"high":"auto"} decoding="async"
