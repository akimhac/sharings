...
export default function NeonButton({ to, children, variant = "primary", className = "" }: Props) {
  const base =
  ...
  if (variant === "ghost") {
    return (
      <Link to={to} className={`${base} border border-white/20 text-white hover:bg-white/5 ${className}`}>
        {children}
      </Link>
    );
  }
